...
export default function Main() {
  ...
  const loadMessages = useCallback(async () => {
    try {
      setIsLoading(true);
      setError(null);
      const [allMessages, count] = await Promise.all([fetchMessages(), fetchMessageCount()]);
      console.log('Messages updated', { allMessages, count });
  ...
  const handleSubmit = useCallback(
    async (event: FormEvent<HTMLFormElement>) => {
      ...
      try {
        ...
        setMessage('');
        console.log('Message posted with hash', hash);
        await loadMessages();
      } catch (err) {
